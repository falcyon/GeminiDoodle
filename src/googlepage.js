...
/**
 * Returns spawn functions for each element group of the Google landing page.
 * Elements are created when their spawn function is called, allowing delayed loading.
 */
export function createGooglePage(world) {
  const W = window.innerWidth / SCALE;
  const H = window.innerHeight / SCALE;

  // Lucky button reference (needed for main.js click handler)
  let luckyButton = null;
  ...
  const logoHH = 5;
  const logoFontSize = 8 * 2 * SCALE * 0.7;
  const logoGap = 0.4;
  const logoY = H * 0.30;
  const totalLogoW = logoLetters.reduce((s, l) => s + l.hw * 2, 0) + logoGap * (logoLetters.length - 1);
  const logoStartX = W / 2 - totalLogoW / 2;

  // ── Spawn functions for each element group ──────────────────────────────
  ...
  function spawnLogoLetter(index) {
    if (index < 0 || index >= logoLetters.length) return;

    let lx = logoStartX;
    for (let i = 0; i < index; i++) {
      lx += logoLetters[i].hw * 2 + logoGap;
    }
  ...
  function spawnSearchButtons() {
    ...
    luckyButton = createStaticBody(world, W / 2 + 15, H * 0.49, 12.5, 2.8, 10);
  ...
  return {
    getLuckyButton: () => luckyButton,
    logoLetterCount: logoLetters.length,
    spawnFooter,
    spawnLogoLetter,
    spawnNav,
    spawnSearchButtons,
